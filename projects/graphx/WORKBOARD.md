...
| W-LEGACY-001 | 迁移前已存在的 GraphX corpus 私有输出实现 | DONE（原任务已落库） | unknown-existing-worktree | implement | graphx WORKTREE | scripts/src/tests 私有输出加固 | 不由本次迁移接管 | unknown | unknown | 全量测试绿 | graphx `1e53457`（已推送）；受保护修改已由原任务提交 |
...
- 受保护的既有修改：GraphX 代码仓当前全部未提交修改；negentropy 的 `backend-notes.md`、`open-questions.md` 以及测试文档删除状态。
