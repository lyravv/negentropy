...
**按需代码审查（reviewer）**：阶段 4 不设强制 review 门禁。`reviewer`（代码审查员）作为可选独立审查者，只在用户/项目负责人明确要求（如"review 一下"、"审查这段代码"、"帮我找冗余/失效代码"）时，才由 orchestrator 路由执行，产出 `04-implementation/code-review.md` 供实现者整改与负责人决策。默认不启动，避免无谓消耗 token 与上下文。
