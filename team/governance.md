...
> **`reviewer` 角色是"审查者"职责的具象化（按需）**：`team.yaml#roles` 中的 `reviewer`（代码审查员）审查代码可读性、复杂度、冗余与失效代码，属于"审查者"这一类职责的一种落地。它 **按需触发**（`on_demand: true`），不进入每次变更的强制门禁：只有用户/项目负责人明确要求 review 时，才由 orchestrator 路由执行。它仍不替代批准者，只产出 review evidence。「审查者」不限于 `reviewer` 一个角色——任何角色在对应交付物上都可能承担 review 职责（见下方批准矩阵的"必须审查"列）。
